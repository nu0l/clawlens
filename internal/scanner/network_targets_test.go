...
func TestScanTargetNetworkDoesNotFlagNonOpenClawServiceAsCritical(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Server", "custom-app")
		w.WriteHeader(http.StatusOK)
	...
	if findings[0].Severity != Warning {
		t.Fatalf("expected Warning for non-openclaw service, got %v", findings[0].Severity)
	}
}
